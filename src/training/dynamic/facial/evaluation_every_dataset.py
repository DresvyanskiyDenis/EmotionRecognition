...
def get_CCC_data_loaders_for_dataset(dataset_name:str, window_size:int):
...
def get_normal_data_loaders_for_dataset(dataset_name:str, window_size:int):
    # load data and separate it on videos
    train, dev, test = load_one_dataset(dataset_name)
    train, dev, test = separate_various_videos(train), separate_various_videos(dev), separate_various_videos(test)
    # construct data loaders
    ...
    for i in tqdm(range(len(info))):
        ...
        for dataset_name in datasets:
            n_train_generator, n_dev_generator, n_test_generator = \
                get_normal_data_loaders_for_dataset(dataset_name=dataset_name, window_size=info['sequence_length'].iloc[i])
            ccc_train_generator, ccc_dev_generator, ccc_test_generator = \
                get_CCC_data_loaders_for_dataset(dataset_name=dataset_name, window_size=info['sequence_length'].iloc[i])
