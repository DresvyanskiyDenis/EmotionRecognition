...
def split_static_dataset_into_train_dev_test(dataset: pd.DataFrame, percentages: Tuple[int, int, int],
...
def split_dataset_into_train_dev_test(filenames_labels: pd.DataFrame, percentages: Tuple[int, int, int],
    ...
    if "AFEW-VA" in filenames_labels['path'].iloc[0]:
        ...
        filenames = filenames_labels['path'].apply(
            lambda x: x.split(os.path.sep)[-3])  # pattern: ParticipantID (DC, JE, JK, or KL)
    else:
...
def load_all_dataframes(seed: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    """
    path_to_AFEW_VA = r"/work/home/dsu/Datasets/AFEW-VA/AFEW-VA/AFEW-VA/preprocessed_pose".replace("\\", os.sep)
    path_to_SEWA = r"/work/home/dsu/Datasets/SEWA/preprocessed_pose".replace("\\", os.sep)
    ...
    path_to_RAD_DB = r"/work/home/dsu/Datasets/RAF_DB/preprocessed_pose"
    path_to_EMOTIC = r"/work/home/dsu/Datasets/EMOTIC/cvpr_emotic/cvpr_emotic/preprocessed_pose/"
    path_to_ExpW = r"/work/home/dsu/Datasets/ExpW/ExpW/preprocessed_pose"

    RAF_DB = pd.read_csv(os.path.join(path_to_RAD_DB, "labels.csv"))  # columns = abs_path,arousal,valence,category
    EMOTIC_train = pd.read_csv(
        os.path.join(path_to_EMOTIC, "train_labels.csv"))  # columns = abs_path,arousal,valence,category
    EMOTIC_dev = pd.read_csv(
        os.path.join(path_to_EMOTIC, "val_labels.csv"))  # columns = abs_path,arousal,valence,category
    EMOTIC_test = pd.read_csv(
        os.path.join(path_to_EMOTIC, "test_labels.csv"))  # columns = abs_path,arousal,valence,category
    ...
    RAF_DB_train, RAF_DB_dev, RAF_DB_test = transform_emo_categories_to_int(RAF_DB_train,
                                                                            training_config.EMO_CATEGORIES), \
    ...
    EMOTIC_train, EMOTIC_dev, EMOTIC_test = transform_emo_categories_to_int(EMOTIC_train,
                                                                            training_config.EMO_CATEGORIES), \
    ...
    train = pd.concat([AFEW_VA_train, SEWA_train, ExpW_train, EMOTIC_train, RAF_DB_train])
    dev = pd.concat([AFEW_VA_dev, SEWA_dev, ExpW_dev, EMOTIC_dev, RAF_DB_dev])
    test = pd.concat([AFEW_VA_test, SEWA_test, ExpW_test, EMOTIC_test, RAF_DB_test])
...
def load_data_and_construct_dataloaders(model_type: str, batch_size: int,
    ...
    if model_type not in ["Modified_HRNet"]:
        raise ValueError(
            'The model type should be "Modified_HRNet".')
    # load pd.DataFrames
